...
async def build_adaptive_graph() -> StateGraph:
    ...
    adaptive_config = await load_yaml_file_async(PROMPTS_CONFIG_PATH)
    ...
    graph.add_node(REQUEST_PARSER, make_rp_node(config=config))
    graph.add_node(CONVERSATIONAL_HANDLER, make_ch_node(llm_model=config["agents"][CONVERSATIONAL_HANDLER]["llm"]))
    graph.add_node(RECON_EXECUTOR, make_re_node(llm_model=config["agents"][RECON_EXECUTOR]["llm"], tools=mcp_tools))
    graph.add_node(RESULT_INTERPRETER, make_ri_node(llm_model=config["agents"][RESULT_INTERPRETER]["llm"], tools=mcp_tools))
    ...
    graph.add_edge(START, REQUEST_PARSER)
    graph.add_edge(REQUEST_PARSER, CONVERSATIONAL_HANDLER)
